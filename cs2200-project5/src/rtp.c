...
packet_t *packetize(char *buffer, int length, int *count)
{
    ...
    *count = num;
    packet_t *packets = malloc(sizeof(packet_t) * (unsigned int)num);
    ...
    do
    {
        ...
        do
        {
            ...
            if (packet.type == DATA || packet.type == LAST_DATA)
            {
                ...
                int chksum = checksum(packet.payload, packet.payload_length);
                if (chksum == packet.checksum)
                {
                    res->type = ACK;
                    int og_length = buffer_length;
                    buffer_length += packet.payload_length;
                    buffer = realloc(buffer, (size_t)buffer_length);
                    memcpy(buffer + og_length, packet.payload, (size_t)packet.payload_length);
                }
                else
                {
                ...
                net_send_packet(connection->net_connection_handle, res);
                free(res);
            }
        ...
        if (connection->alive == 1)
        {
            ...
            char *message_buff = (char *)malloc(sizeof(char) * (unsigned int)(buffer_length));
            memset(message_buff, 0, sizeof(char) * (unsigned int)buffer_length);
            memcpy(message_buff, buffer, sizeof(char) * (unsigned int)buffer_length);
            message->buffer = message_buff;
