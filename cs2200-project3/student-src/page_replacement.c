...
pfn_t free_frame(void) {
    ...
    fte_t* victim_fte = (fte_t*) (victim_pfn + frame_table);

    if (frame_table[victim_pfn].mapped) {

        pcb_t *victim_pcb = victim_fte -> process;
        pte_t *victim_pte = ((pte_t*) ((victim_pcb -> saved_ptbr * PAGE_SIZE) + mem)) + victim_fte -> vpn;
...
pfn_t select_victim_frame() {
    ...
    if (replacement == RANDOM) {
        ...
        for (pfn_t i = 0; i < 2 * NUM_FRAMES; i++) {
            if (!frame_table[ptr].protected && !frame_table[ptr].referenced && frame_table[ptr].timestamp == 0) {
                pfn_t temp = ptr;
