...
/*  --------------------------------- PROBLEM 2 --------------------------------------
    ...
    In this problem, you will initialize frame_table. This global variable is the 
    pointer to the first frame table entry.
    ...
    this location in memory. You should zero out the entries in the frame table, in 
    case for any reason physical memory is not clean.
...
/*  --------------------------------- PROBLEM 3 --------------------------------------
    ...
    you will need to initialize the diskmap which stores the disk addresses of the 
    original blocks (pages) of your object program to be run.
    ...
    HINTS:
        ...
        in the frame_table as protected after we allocate a frame for our page table.
    -----------------------------------------------------------------------------------
*/
void proc_init(pcb_t *proc) {
    ...
    /*
     * 3. Initialize the process' diskmap using diskmap_init()
     */
...
/*  --------------------------------- PROBLEM 4 --------------------------------------
    ...
    Swaps the CPU from the currently running process to the process referenced by the
    proc argument.
...
/*  --------------------------------- PROBLEM 5 --------------------------------------
    ...
    2) Go into physical memory and read/write the data at the translated address.
    Parameters:
...
uint8_t mem_access(vaddr_t address, char rw, uint8_t data) {
    ...
    /* If the entry is invalid, call page_fault() to read in and map a valid page for the page table entry. 
       After that, your page table entry will be valid so that you can proceed. */
    ...
    */
    ...
    paddr_t paddr = (paddr_t) (offset + (pfn * PAGE_SIZE));
...
void proc_cleanup(pcb_t *proc) {
    ...
    pte_t *t_pte = (pte_t*) ((proc -> saved_ptbr * PAGE_SIZE) + mem);
